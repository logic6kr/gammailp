...
class Distance_predicate(nn.Module, BaseRule):
    def __init__(self, train_path, valid_path, test_path, epoch=10, batch_size =512, early_stop = 5, fine_tune = False, append_path='', current_dir = '', task_name='', data_info = '', lr = 5e-5):
    ...
    def embedding_find_fine_tune(self):
        # load train data
        train_data_E = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/train_{self.data_info}_cls_embeddings_token_E_10.hf")
        train_data_R = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/train_{self.data_info}_cls_embeddings_token_R_10.hf")
        train_data_L = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/train_{self.data_info}_cls_embeddings_token_L_10.hf")
        
        valid_data_E  = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/valid_{self.data_info}_cls_embeddings_token_E_10.hf")
        valid_data_R  = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/valid_{self.data_info}_cls_embeddings_token_R_10.hf")
        valid_data_L  = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/valid_{self.data_info}_cls_embeddings_token_L_10.hf")
        
        test_data_E = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/test_{self.data_info}_cls_embeddings_token_E_10.hf")
        test_data_R = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/test_{self.data_info}_cls_embeddings_token_R_10.hf")
        test_data_L = load_from_disk(f"{self.current_dir}/cache/{self.task_name}/test_{self.data_info}_cls_embeddings_token_L_10.hf")
        ...
        # todo: check 
        criterion = nn.MarginRankingLoss(margin=100, reduction='sum')
        ...
        for single_epoch in range(self.epoch):
            ...
            for batch in tqdm.tqdm(train_data_con):
                ...
                loss = criterion(distance, negative_distance, -1*torch.ones_like(distance))
    ...
    def save(self, model, optimizer, append_path=''):
        ...
        unwrapped_model.save_pretrained(
            self.output_model_path_basic+append_path,
...
class DifferentiablePropositionalization(nn.Module, BaseRule):
    ...
    def soft_pro(self):
        ...
        logic_model = DeepRuleLayer_v2(number_relations*self.number_variable_terms, rule_number = 2, default_alpha = 10, output_rule_file=f"{self.current_dir}/cache/{self.task_name}/rule_output.txt", t_relation=f'{self.task_name}', task=f'{self.task_name}', t_arity=2,time_stamp='' )
        logic_model
        
        # define optimizer 
        logic_optimizer = torch.optim.Adam(logic_model.parameters(), lr=0.001)
        best_acc_train  = 0 
        predicate, logic_model = self.accelerator.prepare(predicate, logic_model)
        
        for batch_index, single_bath in enumerate(relation_data):
            if batch_index > 0:
                raise ValueError("Only one batch is allowed")
            single_bath = {k: v for k, v in single_bath.items()}
        ...
        for single_epoch in range(self.epoch):
            for batch_index in tqdm.tqdm(substation_data):
                ...
                first_entity_id = batch['first_entity_id']  # obj entity 
                first_entity_mask = batch['first_entity_mask']
                second_entity_id = batch['second_entity_id'] # label entity / sub entity / second entity 
                second_entity_mask = batch['second_entity_mask']
                third_entity_id = batch['third_entity_id']
                third_entity_mask = batch['third_entity_id']
...
def pre_train_predicate_neural():
    ...
    model = Distance_predicate(train_path, valid_path, test_path, epoch=2, batch_size =32, data_info=data_info, fine_tune = False, current_dir=current_dir, task_name=task_name, early_stop=False)
    model.embedding_find_fine_tune()
    # model.inference_distance(model_path = 'gammaILP/cache/icews14/out/2511/bert_fine_tune_balance.pt')
