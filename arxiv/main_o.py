...
class DifferentiablePropositionalization(nn.Module, BaseRule):
    def __init__(self, entity_path, relation_path, epoch=10, batch_size =512, early_stop = 5, ele_number=10, current_dir='',device='', task_name='',final_dim = 10,threshold = 1500, train_loop_logic=10, model_name_specific = None, tokenize_length=10, load_pretrain_head=True, lr_predicate_model = 1e-4, lr_rule_model = 1e-3, lr_dkm  = 1e-4, target_predicate = 'Host a visit', data_format = 'kg', cluster_numbers = 10, alpha=1, number_variable=3, target_variable_arrange = 'X@Y', target_variable_arrange_index = (0,1), stop_recall = 0.97):
        ...
        self.rule_path = f"{self.current_dir}/cache/{self.task_name}/rule_output_{self.target_relation}_o.md"
        self.writer = SummaryWriter(f'{self.current_dir}/cache/{self.task_name}/runs/')
        if model_name_specific == None:
            self.datetime = datetime.datetime.now().strftime("%d%H")
        else: 
    ...
    def rule_body_train(self):
        ...
        self.entity_entityindex = self.get_connected_entity(train_data)
...
# if __name__ == "__main__":
    ...
    get_index_X_Y = args.target_variable_arrange_index.split(',')
    get_index_X_Y = (int(get_index_X_Y[0]), int(get_index_X_Y[1]))
    #initial the model
    model = DifferentiablePropositionalization(entity_path, relation_path, epoch=args.epoch, batch_size =2, current_dir=args.folder_name, device=args.device,task_name = args.task, threshold=5000, train_loop_logic=args.train_loop_logic, model_name_specific=args.model_path, ele_number=args.element_number, tokenize_length=args.tokenize_length, load_pretrain_head=args.load_pretrain_head, lr_predicate_model=args.lr_predicate, lr_rule_model=args.lr_rule, target_predicate=args.target_predicate, data_format=args.data_format, cluster_numbers=args.cluster_numbers, lr_dkm = args.lr_dkm, alpha = args.alpha, early_stop=args.early_stop, number_variable=args.number_variable, target_variable_arrange_index=get_index_X_Y, target_variable_arrange=args.target_variable_arrange,stop_recall=args.stop_recall)
    model.make_tokeniz()
    # model.soft_pro()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr_dkm', type=float, default=1)
    ...
    parser.add_argument('--cluster_numbers', type=int, default=6, help='The number of the clusterings centroid')
    ...
    parser.add_argument('--target_predicate', type=str, default='adjr', help='target predicate')
    parser.add_argument("--task", type=str, default='adjr')
    ...
    parser.add_argument('--target_variable_arrange_index', type=str, default='0,0', help='the target variable arrange for the target predicate')
