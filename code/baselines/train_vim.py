...
def get_args():
    ...
    parser.add_argument(
        ...
        choices=["twopairs", "threepairs", "red-triangle", "closeby",
                 "online", "online-pair", "nine-circles", "onered", "onetriangle"],
        default="onered",
...
def predict(net, loader, device, th=None):
    predicted_list = []
    target_list = []
    count = 0
    for i, sample in tqdm(enumerate(loader, start=0)):
        ...
        predicted = get_classification(net(imgs))
...
def run(net,  loader, optimizer, criterion, writer, args, device, train=False, epoch=0,  rtpt=None, max_obj_num=4):
    ...
    for i, sample in tqdm(enumerate(loader, start=epoch * iters_per_epoch)):
        ...
        x = get_classification(net(imgs))
        
        # binary cross-entropy loss computation
        loss = be(x, target_set)
        loss.backward()
        loss_sum += loss.item()
...
def get_classification(x):
    """Get classification from the output of the model.
    Args:
        x (torch.Tensor): Output of the model.
    Returns:
        torch.Tensor: Classification of the output.
    """

    possibility_true = torch.softmax(x, dim=1)[:, 1]
    return possibility_true.float()
...
def main(n):
    ...
    train_loader, val_loader, test_loader = get_data_loader(args, IMAGE_SIZE=224)
    ...
    for param in params:
        if not param.requires_grad:
            print("Setting requires_grad to True for parameter: ", param)
            param.requires_grad = True
    ...
    for epoch in np.arange(start_epoch, args.epochs + start_epoch):
        # training step
        loss = run(
            net, train_loader, optimizer, criterion, writer, args, device=device, train=True, epoch=epoch, rtpt=rtpt)
