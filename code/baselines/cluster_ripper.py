import pickle, torch, itertools, datetime, argparse, datasets, tqdm, sys, os
...
class DifferentiablePropositionalization(nn.Module, BaseRule):
    def __init__(self, entity_path, relation_path, epoch=10, batch_size =512, early_stop = 5, ele_number=10, current_dir='',device='', task_name='',final_dim = 10,threshold = 1500, train_loop_logic=10, model_name_specific = None, tokenize_length=10, load_pretrain_head=True, lr_predicate_model = 1e-4, lr_rule_model = 1e-3, lr_dkm  = 1e-4, target_predicate = 'Host a visit', data_format = 'kg', cluster_numbers = 10, alpha=1, number_variable=3, target_variable_arrange = 'X@Y', stop_recall = 0.97, substitution_method = 'random', random_negative = 32, open_neural_predicate = False, output_file_name = '', minimal_precision = 0.5, body_arity = '',lambda_dkm = 1):
        ...
        if self.data_format == 'image' or self.data_format == 'pi' or self.data_format == 'ppi':
    ...
    def predicate_invention(self, inner_batch = 2):
        ...
        test_data_neg = torch.load(f"{self.current_dir}/cache/{self.task_name}/negative_images_data_test.pt")
        all_obj_bert_embeddings = torch.concat([train_data_pos, train_data_neg], dim=0).to(self.device) # get all true index of object
        all_bert_embeddings_test = torch.concat([test_data_pos, test_data_neg], dim=0).to(self.device) # get all true index of object
        # load the index to image file 
        with open(f"{self.current_dir}/cache/{self.task_name}/index_to_image_train.pkl", 'rb') as f:
            self.index_to_image = pickle.load(f)
            f.close()
        ...
        # reshape embedding into -1 768
        all_obj_bert_embeddings_numpy = all_obj_bert_embeddings.view(-1, self.bert_dim_length).cpu().numpy()  # Reshape to (-1, 768)
        all_obj_bert_embeddings_test_numpy = all_bert_embeddings_test.view(-1, self.bert_dim_length).cpu().numpy()  # Reshape to (-1, 768)
        print('[pretraining done]')
        
        # get the train and test instnace number 
        number_train_instance = all_obj_bert_embeddings.shape[0]
        number_test_instance = all_bert_embeddings_test.shape[0]
        all_obj = np.concatenate((all_obj_bert_embeddings_numpy, all_obj_bert_embeddings_test_numpy), axis=0)
        kmeans_model = KMeans(n_clusters=self.n_clusters, init="k-means++").fit(all_obj)
        print('[kmeans done]')
        print('[K-Means prediction for embeddings]')
        all_obj_symbolic_representation = kmeans_model.predict(all_obj)
        print(all_obj_symbolic_representation)
        # self.plot_cluster(all_obj_bert_embeddings_numpy, cluster_labels = all_obj_symbolic_representation, addition_inputs = 'init_')

        
        # type here for ripper and c4.5 implementation based on the clustering results
        # reshape to (number of images,  4)
        all_obj_symbolic_representation = all_obj_symbolic_representation.reshape(-1, 4)
        
        train_obj = all_obj_symbolic_representation[:number_train_instance,:]
        test_obj = all_obj_symbolic_representation[number_train_instance:,:]
        print(train_obj)
        print(test_obj)
        
        # turn the train_obj and test_obj to numpy, with 9 column, each elemetns indicate the cluster index is in the train_obj or not 
        train_obj_final = np.zeros((train_obj.shape[0], self.n_clusters))
        test_obj_final = np.zeros((test_obj.shape[0], self.n_clusters))
        for i in range(train_obj.shape[0]):
            for j in range(train_obj.shape[1]):
                train_obj_final[i, train_obj[i,j]] = 1
        for i in range(test_obj.shape[0]):
            for j in range(test_obj.shape[1]):
                test_obj_final[i, test_obj[i,j]] = 1
        
        # get the positive labels 
        train_labels = torch.tensor([1]*train_data_pos.shape[0] + [0]*train_data_neg.shape[0], device=self.device)
        test_labels = torch.tensor([1]*test_data_pos.shape[0] + [0]*test_data_neg.shape[0], device=self.device)
        
        # turn labels to numpy 
        train_labels_numpy = train_labels.cpu().numpy()
        test_labels_numpy = test_labels.cpu().numpy()
        
        # call decision tree model 
        clf = DecisionTreeClassifier(random_state=0)
        clf.fit(train_obj_final, train_labels_numpy)

        acc = clf.score(test_obj_final, test_labels_numpy)
        print('Accuracy of decision tree classifier:', acc)
        # save the rules to the results path 
        with open(f"{self.current_dir}/cache/{self.task_name}/c4_5.md", 'w') as f:
            f.write(export_text(clf, feature_names=[f'V{i}' for i in range(train_obj_final.shape[1])]))
            f.write(f'Accuracy of decision tree classifier: {acc}')
            f.close()
        ...
        clf.fit(train_obj_final, train_labels_numpy)
        acc = clf.score(test_obj_final, test_labels_numpy)
        print('Accuracy of ripper classifier:', acc)
        with open(f"{self.current_dir}/cache/{self.task_name}/ripper.md", 'w') as f:
            f.write(str(clf.ruleset_))
            f.write(f'Accuracy of ripper classifier: {acc}')
            f.close()
        
        # plot the clsuter index and the original pictures 
        self.plot_cluster(all_obj_bert_embeddings, cluster_labels = all_obj_symbolic_representation.reshape(-1).tolist()[:number_train_instance], addition_inputs = 'ripper_c45')
        return 0
        ...
        for single_epoch in range(self.epoch):
            ...
            # build on test data
            # infer_time = self.infer(infer_time)
            self.plot_cluster(all_obj_bert_embeddings)
            total_loss_rule = total_loss_rule / len(substation_data)
            total_loss = total_loss / len(substation_data)
            total_dkm_loss = total_dkm_loss / len(substation_data)
            print(f"Epoch: {single_epoch}, Loss: {total_loss}, Body Loss: {total_body_loss}, Rule Loss: {total_loss_rule}, DKM Loss: {total_dkm_loss}, DKM acc, {dkm_acc}")
    ...
    def plot_cluster(self, all_obj_bert_embeddings, top_n = 20, cluster_labels = None, addition_inputs = ''):
        selected_embeddings  = all_obj_bert_embeddings.reshape(-1,self.bert_dim_length)[:top_n]
        ...
        for label, paths in label_to_paths.items():
            ...
            plt.tight_layout()
            plt.savefig(f"{self.current_dir}/cache/{self.task_name}/{addition_inputs}cluster_{[label]}.png")
        return 0
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--target_predicate', type=str, default='even', help='target predicate, can be lessthan or predicate inside the dataset')
    # parser.add_argument("--task", type=str, default='even_m', help='task name. Choose from mnist, relational_images, even, lessthan, kandinsky_onered_all, etc')
    # parser.add_argument('--data_format', type=str, default='image', help='Can be chosen from kg or image or "pi" (predicate invention)')
    # parser.add_argument('--cluster_numbers', type=int, default=10, help='The number of the clusterings centroid')
    # parser.add_argument('--element_number', type=int, default= -1, help='number of elements to sample for training, if the number is less than 0, then the number of elements is the number of all elements')
    # parser.add_argument('--alpha', type=int, default=20, help='the alpha value for the differentiable k-means') 
    ...
    # # # ! config for ILP  
    # parser.add_argument('--target_predicate', type=str, default='fizz', help='target predicate, can be lessthan or predicate inside the dataset')
    # parser.add_argument("--task", type=str, default='fizz', help='task name. Choose from mnist, relational_images, even, lessthan, kandinsky_onered_all, etc')
    # parser.add_argument('--data_format', type=str, default='kg', help='Can be chosen from kg or image or "pi" (predicate invention)')
    # parser.add_argument('--cluster_numbers', type=int, default=10, help='The number of the clusterings centroid')
    # parser.add_argument('--element_number', type=int, default= -1, help='number of elements to sample for training, if the number is less than 0, then the number of elements is the number of all elements')
    # parser.add_argument('--alpha', type=int, default=20, help='the alpha value for the differentiable k-means') 
    # parser.add_argument('--target_variable_arrange', type=str, default='X@X', help='the target variable arrange for the target predicate')
    # parser.add_argument('--number_variable', type=int, default=4, help='the number of variables in the logic program')
    # parser.add_argument('--stop_recall', type=float, default=1.1, help='the target relation for the task')
    # parser.add_argument('--minimal_precision', type=float, default=0.5, help='the least precision the rules in the learned ruleset')
    # parser.add_argument('--substitution_method', type=str, default='chain_random', help='the method to generate the substitution for the body predicates, choose from random, all, chain_random')
    # parser.add_argument('--random_negative', type=int, default=4, help='Number of batch z when using random substitution method')
    ...
    parser.add_argument('--target_predicate', type=str, default='target', help='target predicate, can be lessthan or predicate inside the dataset')
    parser.add_argument("--task", type=str, default='kandinsky_twopairs_50', help='task name. Choose from mnist, relational_images, even, lessthan, kandinsky_onered_all, etc')
    parser.add_argument('--data_format', type=str, default='pi', help='Can be chosen from kg or image or pi')
