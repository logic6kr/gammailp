...
def get_data_loader(args, IMAGE_SIZE=128):
    if args.dataset_type == 'kandinsky':
        return get_kandinsky_loader(args, IMAGE_SIZE=IMAGE_SIZE)
...
def get_kandinsky_loader(args, shuffle=False, IMAGE_SIZE=128):
    dataset_train = data_kandinsky.KANDINSKY(
        args.dataset, 'train', small_data=args.small_data, img_size=IMAGE_SIZE
    )
    dataset_val = data_kandinsky.KANDINSKY(
        args.dataset, 'val', small_data=args.small_data, img_size=IMAGE_SIZE
    )
    dataset_test = data_kandinsky.KANDINSKY(
        args.dataset, 'test', img_size=IMAGE_SIZE
    )
    train_loader = torch.utils.data.DataLoader(
        dataset_train,
        shuffle=True,
