...
class WikidataRelationStatements_Base(RelationStatements):
    ...
    def load_raw_split(self, split_name):
        ...
        split_file = self.conf.datadir / self.conf.task_name / fname
        if split_name in {'dev', 'test'}:
            if split_name == 'test' and self.conf.test_data_path != None:
                split_file = self.conf.test_data_path
                # set max inst to the maximize
                n_inst = sys.maxsize
            elif split_name == 'test' and self.conf.max_test_inst:
...
class WikidataRelationStatements(WikidataRelationStatements_Base, TrainDevTest):
