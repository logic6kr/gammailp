...
from data import WikidataRelationStatements
import json
...
class TrainerBase():
    ...
    def test(self):
        ...
        but do not perform any training."""
        # cache_file = 'cache/'
        # # clean all files in cache folder 
        # for f in os.listdir(cache_file):
        #     os.remove(os.path.join(cache_file, f))
        # check if cache not empty, then rasie error 
        # if len(os.listdir(cache_file)) != 0:
        #     raise Exception("Cache folder is not empty")
        self.start_run()
    ...
    def inference(self, data:list[str]=[]):
        ...
        infer_data_path =  'data/wikidata_relation_statements/infer.jl'
        raw_data = []
        with open(infer_data_path) as f:
            for line in f:
                raw_data.append(json.loads(line))
        instance = raw_data_to_instance(raw_data)
        token_isntance = tensorize_split(instance)
        infer_loader  = DataLoader(token_isntance, batch_size=5, shuffle=False)
        # run the model
        self.eval_engine.run()
        self.log_metrics()
        self.log_results('inference', self.eval_engine.state.metrics)
        self.save_results()
        self.end_run()
        if self.is_dist_main():
            return self.results
