...
from aix360_k.aix360.algorithms.rule_induction.trxf.core.utils import batch_evaluate

RuleContributionMetrics = namedtuple(
    'RuleContributionMetrics',
    'conjunction, tp_common, tp_unique, fp_common, fp_unique, n_correct, n_records, laplace_estimate, precision, recall, tp, tn, fp, fn,lift')
"""
...
def compute_confusion_matrix(ruleset: DnfRuleSet, X: pd.DataFrame, y: pd.Series) -> List[RuleContributionMetrics]:
    y_pred = batch_evaluate(ruleset, X)
    tp, tn, fp, fn = get_preaggregated_confusion_matrix(y_pred, y, ruleset.then_part)
    try:
    ...
    try:
        ...
        tnr = 0
    return len(tp), len(tn), len(fp), len(fn), precision, recall, tnr
...
def compute_rule_metrics(ruleset: DnfRuleSet, X: pd.DataFrame, y: pd.Series) -> List[RuleContributionMetrics]:
    ...
    then_part = ruleset.then_part
    res = []
    number_negative = len(y[y == 0])
    number_positive = len(y[y == 1])
    positive_ratio = number_positive / (number_positive + number_negative)
    for conjunction in tqdm.tqdm(ruleset.conjunctions, desc='Computing rule metrics'):
        ...
        try:
            precision = len(tp_conj) / (len(tp_conj) + len(fp_conj))
        except:
            precision = 0
        try:
            recall = len(tp_conj) / (len(tp_conj) + len(fn_conj))
        except:
            recall = 0
        ...
        res.append(RuleContributionMetrics(
            conjunction, tp_common, tp_unique, fp_common, fp_unique, n_correct, n_records, laplace_estimate,precision,recall,len(tp_conj),len(tn_conj),len(fp_conj),len(fn_conj),lift))
    return res
