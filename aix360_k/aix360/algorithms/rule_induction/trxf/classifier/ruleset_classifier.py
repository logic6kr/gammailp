...
from aix360_k.aix360.algorithms.rule_induction.trxf.metrics import compute_rule_metrics
import tqdm

Rule = namedtuple('Rule', ['conjunction', 'label', 'record_count', 'nb_correct', 'confidence', 'weight','precision','recall','tp','tn','fp','fn','lift'])
...
class RuleSetClassifier:
    ...
    def __init__(self,
        ...
        self._rules = [Rule(conjunction=c,
                            ...
                            confidence=None,
                            weight=None, 
                            precision=None,recall = None, tp=None, fp = None, tn=None, fn=None, lift = None)
    ...
    def update_rules_with_metrics(self, X: pd.DataFrame, y: pd.Series) -> None:
        ...
        for rule_set in self._rule_sets:
            ...
            for rule in metric_list:
                ...
                precision = self._compute_precision(rule)
                recall = self._compute_recall(rule)
                tp = rule.tp
                tn = rule.tn 
                fp = rule.fp
                fn = rule.fn
                ...
                new_rules.append(
                    Rule(conjunction=rule.conjunction, label=rule_set.then_part, record_count=rule.n_records,
                         nb_correct=rule.n_correct, confidence=confidence, weight= weight,precision=precision, recall = recall, tp=tp,tn=tn,fp=fp,fn=fn,lift=lift))
    ...
    def remove_low_precision(self, lower_threshold = 0.5):
        current_rule = self._rule_sets[0]
        remove_list = []
        remove_index = 0
        for rule in self.rules:
            if rule.precision < lower_threshold:
                remove_list.append(remove_index)
            remove_index += 1
        new_rule = copy.deepcopy(current_rule)
        for item in remove_list:
            target_conjunction = current_rule.conjunctions[item]
            new_rule.remove_conjunction(target_conjunction)
        new_rule={'new_rule':new_rule, 'removed_rules':remove_list}
        return new_rule
    ...
    def _compute_recall(self,rule):
        recall = rule.recall
        return recall
    
    def _compute_precision(self,rule):
